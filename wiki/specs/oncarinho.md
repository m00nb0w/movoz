...
```
players
  id, name, position (nullable text), is_active (bool, default true), created_at
...
GET /api/players/:id                              — profile: all-time totals + year-by-year breakdown
GET /api/leaderboard?year=YYYY|all&stat=goals|assists|cards  — ranked table
GET /api/summary                                  — team dashboard totals (current year)
...
PUT  /api/matchdays/:id/stats             — bulk upsert per-player stats for that matchday
```

`ADMIN_PASSWORD` is an env var, consistent with other backend services in this repo. Auth middleware guards all admin-prefixed routes and returns 401 on a missing/invalid session cookie.
...
| `/admin/players` | Add/edit/deactivate roster |

## Error Handling & Testing

- Validation: player `name` required; matchday `played_on` required. Admin write endpoints return 400 on invalid input, 401 on missing/invalid session.
