...
- [ ] **Deployment is deferred to a separate follow-up plan** (same call `oncarinho` made). Scout runs locally only today: `pnpm --filter @movoz/scout dev` plus `go run ./cmd/server` in `backend/scout`, with the frontend's dev-mode rewrite in `apps/scout/next.config.mjs` proxying `/scout/api/*` to the backend. Nothing about Scout is containerised or routed in production. That plan must land three things together, because any one alone is broken: (1) Dockerfiles for `apps/scout` and `backend/scout`, (2) `scout` + `scout-backend` services in `infra/docker-compose.yml` (plus a Postgres for Scout), and (3) the `/scout`, `/scout/_next`, and `/scout/api` routes in `infra/nginx/nginx.conf`. The nginx routes were reverted in this pass (see the TODO block at the top of that file) precisely because they shipped without (1) and (2): nginx resolves upstream hostnames at config-load time, so referencing a non-existent `scout` service is a fatal `[emerg] host not found in upstream` that broke `docker compose up` for the entire stack, personal-site included. That file's TODO also records the SSE proxy settings (`proxy_buffering off`, `proxy_http_version 1.1`, `proxy_read_timeout 300s`) the `/scout/api` route will need, or the AI ranking chat (F9) will buffer instead of streaming in production.
