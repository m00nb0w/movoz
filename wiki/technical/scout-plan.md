...
### Task 42: Engineer edit UI (closes F2 gap — add/edit/deactivate)

**Files:**
- Modify: `apps/scout/src/app/engineers/page.tsx`

**Interfaces:**
- Consumes: `api.put` (Task 32), `Engineer` (Task 33's `src/lib/types.ts`)
- Produces: no new exports — adds an inline edit affordance to the existing roster page

**Context:** The spec (F2, `wiki/specs/scout.md`) requires "Admin can add, edit, and deactivate players" — sorry, engineers. Task 33 built add/deactivate/reactivate against the backend's `POST /api/engineers`, `DELETE /api/engineers/:id`, `POST /api/engineers/:id/reactivate`, but no task in this plan ever wired up the backend's existing `PUT /api/engineers/:id` (already implemented and tested in Task 3) from the UI. This task closes that gap with an inline edit toggle on the roster row — no new page, no new route, no backend changes.

- [ ] **Step 1: Add inline edit state and a PUT-backed save handler**
```tsx
// apps/scout/src/app/engineers/page.tsx — modify EngineersPage
"use client";
...
Expected: both succeed with no type errors
- [ ] **Step 3: Manual verify**
Run: `pnpm --filter @movoz/scout dev` and open `http://localhost:3002/scout/engineers` — click "Edit" on an existing engineer, change the name/role, save, confirm the row reflects the change after reload; click "Cancel" mid-edit and confirm no changes are sent
- [ ] **Step 4: Commit**
```bash
git add apps/scout/src/app/engineers/page.tsx
git commit -m "scout: add engineer edit UI, closing F2 add/edit/deactivate gap"
```
