...
**Stat-entry view** (after clicking a matchday): a grid/table with one row per active player — columns: Player, Goals, Assists, Yellow Cards, Red Cards, each a small number `Input` (default 0). No per-row remove control — a player left at all zeros simply didn't play: on Save, an all-zero row creates no stat record (so it doesn't count toward "matches played"), and if a player previously had a real row and gets zeroed out, their existing record is deleted (`DELETE .../stats/:playerId`, called automatically, not from a button). A single "Save" `Button` at the bottom submits the whole grid as one bulk upsert (`PUT /api/matchdays/:id/stats`) plus any needed deletes. Pre-fill existing values if editing a previously-entered matchday (`GET /api/matchdays/:id/stats`). This is the single most-used admin screen — optimize for fast keyboard-driven data entry (tab order flows naturally row by row), not visual flourish.
